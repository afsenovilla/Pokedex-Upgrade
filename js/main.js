...
function mostrarPokemon(data) {
    ...
    let tipos = data.types.map((type) => `<p class="`+type.type.name+` tipo">`+nombreTipos.get(type.type.name)+`</p>`);
    tipos = tipos.join('')
...
botonesTipos.forEach(boton => boton.addEventListener("click", () => {
...
botonesRegion.forEach(boton => boton.addEventListener("click", () => {
    const botonId = event.currentTarget.id;
